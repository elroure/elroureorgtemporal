...
const RotatedMenu: React.FC<RotatedMenuProps> = ({
  ...
  return (
    <nav
  className={`flex flex-col ${isMobile || !isOnRight ? 'items-center' : 'items-start'} w-full gap-y-6 ${className}`}
>
  {items.map((item, index) => (
    <Link
      key={index}
      to={item.href || "#"}
      className={`block font-handscript text-[#43362A] text-xl sm:text-2xl xl:text-3xl 2xl:text-4xl leading-relaxed transition-all duration-1000 opacity-${isVisible ? '100' : '0'} hover:text-opacity-70 ${isMobile || !isOnRight ? 'text-center' : 'text-left'}`}
      style={{
        transform: isVisible 

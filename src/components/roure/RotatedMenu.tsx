  ...
  return (
    <nav className={`${className} ${isVisible ? 'block' : 'hidden'} mt-0`}>
      {items.map((item, index) => (
        <div
          key={index}
          className={`block font-handscript text-[#43362A] text-2xl leading-9 rotate-[-15deg] mb-10 max-md:text-center max-sm:text-xl transition-opacity duration-5000 opacity-${isVisible ? '100' : '0'}`}
          style={{

...
const Logo: React.FC<LogoProps> = ({ className, animationDelay = 0 }) => {
  ...
  useEffect(() => {
    ...
    const dateTimer = setTimeout(() => setIsDateLoaded(true), animationDelay + 500);
    ...
    <div className={`flex flex-col items-center ${className}`}>
      <div className="overflow-hidden">
        <img
          ...
          className={`w-[266px] h-[91px] max-sm:w-[200px] max-sm:h-auto transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
          alt="Roure Logo"
          style={{
      ...
      <div 
        className={`font-handscript text-[#43362A] text-2xl mt-5 max-sm:text-xl transition-opacity duration-1000 ${isDateLoaded ? 'opacity-100' : 'opacity-0'}`}
        style={{
